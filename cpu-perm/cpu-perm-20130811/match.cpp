...
int CMP(char * strRead, int len, SIZE_T index, const CReference * refGenome) {
	InBits r, v;
...
SIZE_T LowerBound(SIZE_T low, SIZE_T high, char * strRead, int s, const CReference * refGenome, const CHashTable * hashTable) {
	SIZE_T mid = 0;
	while (low < high) {
		mid = (low + high) / 2;
...
SIZE_T UpperBound(SIZE_T low, SIZE_T high, char * strRead, int s, const CReference * refGenome, const CHashTable * hashTable) {
	SIZE_T mid = 0;
	while (low < high) {
		mid = (low + high + 1) / 2;
		if (CMP(strRead, s, hashTable->index[mid], refGenome) >= 0)
...
int ResultExist(const CResult * oneResult, SIZE_T index) {
...
void Mapping(const MapOpt mapOpt, const CReference * refGenome, const CHashTable * hashTable, CRead oneRead, CResult * oneResult) {
	for (int i = 0; i < NUMBER_OF_SHIFT; i++) {
		...
		//cout << " l u" << lower << " " << upper << endl;
		for (SIZE_T j = lower; j <= upper; j++) {
			//if (ResultExist(oneResult, hashTable->index[j] - i))
			//	continue;
			//cout << "j = " << j << endl;
			SIZE_T s = GetKmer(refGenome, hashTable->index[j] - i, oneRead.readLen, &ret);
...
void MappingOneRead(const MapOpt mapOpt, const CReference * refGenome, const CHashTable * hashTable, CRead oneRead, CResult * oneResult) {
	oneResult->nRet = 0;
