...
bool GetHashValue(const char * strVal, const SIZE_T & strValLen, const SIZE_T & nStartPos, const int & len, SIZE_T & hashValueRet) {
	WORD_SIZE hashValueU = 0, hashValueL = 0;
	int valid = 0;
	SIZE_T i = 0;
	while(valid < len) {
		if(nStartPos + i >= strValLen) return false;
