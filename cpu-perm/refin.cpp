...
SIZE_T SEED_SPACE[BITS_FOR_HASHING] = { 0, 1, 2, 4, 7, 8, 9, 11, 14, 15, 16, 18,
    21 };

bool GetHashValue(const char * strVal, const SIZE_T & strValLen,
                  const SIZE_T & nStartPos, const int & len,
                  SIZE_T & hashValueRet) {
  WORD_SIZE hashValueU = 0, hashValueL = 0;
  int valid = 0;
  SIZE_T i = 0;
  while (valid < len) {
    if (nStartPos + i >= strValLen)
      return false;
  ...
  hashValueRet = (SIZE_T)((hashValueU << len) + hashValueL);
  return true;
}
...
int RemoveNonACGTNBase(char * strRef, SIZE_T refLen,
  ...
  for (SIZE_T i = 0; i < j; i++) {
    if (i % NUMBER_OF_SPACE != 0)
      continue;
    for (SIZE_T k = 0; k < BITS_FOR_HASHING; k++) {
  ...
  ////////////////////////////////////////////////////
  //statistics
  map<SIZE_T, int> statCountBucket;
  SIZE_T n = j;
  int l = 13;
  SIZE_T hashValue = 0;
  for (SIZE_T i = 0; i <= n - l; i++) {
    if (GetHashValue(strRef, n, i, l, hashValue)) {
      statCountBucket[hashValue]++;
    }
  }
  char fileName[100];
  sprintf(fileName, "test/statCountBucket_len_%d.txt", l);
  ofstream fout(fileName);
  for (map<SIZE_T, int>::iterator it = statCountBucket.begin();
      it != statCountBucket.end(); it++) {
...
void GetReference(CReference * refGenome, Option & opt) {
  ...
  refGenome->nRefSize = RemoveNonACGTNBase(strRef, refLen, opt.setConsectiveN);
  cout << refGenome->nRefSize << endl;
  //genTestData(strRef, refGenome->nRefSize);
  RefEncodeToBits(refGenome, strRef);
  cout << " opt.setConsectiveN.size() = " << opt.setConsectiveN.size() << endl;
