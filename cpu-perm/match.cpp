...
void Reverse_Kernel(const CRead * reads, CRead * reads_rev,
  ...
  for (SIZE_T i = 0; i < nReadsNum; i++) {
    reverseCompliment(reads_rev[i].readInStr, reads[i].readInStr,
                      reads[i].readLen);
...
SIZE_T LowerBound(SIZE_T low, SIZE_T high, const char & chr,
                  const SIZE_T & cmpbits, const CReference * refGenome,
...
SIZE_T UpperBound(SIZE_T low, SIZE_T high, const char & chr,
                  const SIZE_T & cmpbits, const CReference * refGenome,
                  const CHashTable * hashTable) {
  SIZE_T mid = 0;
  while (low < high) {
...
void Match(const CReference * refGenome, const CHashTable * hashTable,
           const CRead * read, const SIZE_T & nStartPos, CRegion * ret) {
  ...
  for (SIZE_T i = nStartPos + 22, cmpbits = 22; i < read->readLen;
      i++, cmpbits++) {
    if (F2SEEDPATTERN[cmpbits] == '0') {
      continue;
    }
