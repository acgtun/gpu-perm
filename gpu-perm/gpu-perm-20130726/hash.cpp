...
void MakeHashTable(const RefGenome * refGenome, HashTable * hashTable) {
	...
	hashTable->counter = (SIZE_T *) malloc(
			sizeof(SIZE_T) * (hashTable->NO_OF_BUCKET + 1));
	memset(hashTable->counter, 0x00,
			sizeof(SIZE_T) * (hashTable->NO_OF_BUCKET + 1));
...
void CountBucketSize(const RefGenome * refGenome, HashTable * hashTable) {
	...
	for (SIZE_T i = 1; i <= hashTable->NO_OF_BUCKET; i++) {
		hashTable->counter[i] += hashTable->counter[i - 1];
	}

	for (SIZE_T i = hashTable->NO_OF_BUCKET - 1; i >= 1; i--) {
		hashTable->counter[i] = hashTable->counter[i - 1];
	}
...
void testSort(const RefGenome * refGenome, const HashTable * hashTable) {
	LOG_INFO
	;
	static int cnt = 0;
	InBits r;
	FILE * fsee = fopen("test/test_out_sort.txt", "wb");
	LOG_INFO
	for (SIZE_T i = 0; i < hashTable->NO_OF_BUCKET; i++) {
		int start = hashTable->counter[i];
		if (hashTable->counter[i + 1] - start < 5)
			continue;
		if (i == 0)
			continue;
		if (cnt > 100)
		...
		for (SIZE_T j = start; j < hashTable->counter[i + 1]; j++) {
			...
			fprintf(fsee, "strRead = %s ", strRead);
			fprintf(fsee, " j = %d, i =%d %d\n", j, i, hashTable->index[j]);
		}
		fprintf(fsee, "\n");
	}
	fclose(fsee);
