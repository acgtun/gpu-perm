...
__host__ __device__ int GetF2Seed(WORD_SIZE encode, SIZE_T len, WORD_SIZE * seed) {
...
__host__ __device__ SIZE_T GetKmer(const InBits * refGenome, SIZE_T nRefSize, SIZE_T nRefStart, SIZE_T kmerLen, InBits * r) {
	if (nRefStart + kmerLen > nRefSize) {
		kmerLen = nRefSize - nRefStart;
	}
	//LOG_INFO
	r->ub = 0;
	r->lb = 0;
	...
	r->ub = refGenome[indexInWords].ub >> bitsShift;
	r->lb = refGenome[indexInWords].lb >> bitsShift;
	//LOG_INFO
	...
	if (bitsShift != 0) {
		...
		r->ub |= (refGenome[indexInWords + 1].ub << (wordSize - bitsShift));
		r->lb |= (refGenome[indexInWords + 1].lb << (wordSize - bitsShift));
	}
	//LOG_INFO
