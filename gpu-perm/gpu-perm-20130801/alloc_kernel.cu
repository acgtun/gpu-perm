...
void run_kernel(FILE * fout, int nMaxMismatch, const InBits * refGenome, SIZE_T nRefSize, const SIZE_T * hashCounter, const SIZE_T * hashIndex,
		InBits * d_reads, SIZE_T * d_readsLen,  ReadsMatch * readsMatch, ResultMatchedReads * d_result, SIZE_T * readID, ResultMatchedReads * result)
{
	...
	gpu_kernel<<<BLOCKS, THREADS>>>(nMaxMismatch, refGenome, nRefSize, hashCounter, hashIndex, d_reads, d_readsLen,
...
void ReadReads(const Option & opt, const InBits * refGenome, SIZE_T nRefSize, const SIZE_T * hashCounter, const SIZE_T * hashIndex) {
	...
	for (SIZE_T i = 0; i < readsLen; i++) {
		...
		if (readsMatch.nReads >= MAX_MAPPING_READS) {
			run_kernel(fout, opt.matchOpt.nMaxMismatch, refGenome,  nRefSize,  hashCounter, hashIndex, d_reads,  d_readsLen, &readsMatch, d_result,& readID,result);
			//HANDLE_ERROR( cudaMemcpy(d_reads, readsMatch.readsInBits, (readsMatch.nReads) * sizeof(InBits), cudaMemcpyHostToDevice));
			//HANDLE_ERROR( cudaMemcpy(d_readsLen, readsMatch.readsLen, (readsMatch.nReads) * sizeof(SIZE_T), cudaMemcpyHostToDevice));
