...
void MemoryAlloocate(RefGenome * d_refGenome, HashTable * d_hashTable,
		ReadsMatch * d_readsMatch, RefGenome * refGenome,
		HashTable * hashTable) {
...
void run_kernel(FILE * fout, ReadsMatch * readsMatch,
	...
	printf("size = %d\n", size);
	for (int i = 0; i < 10; i++) {
		cout << "here = " << i << " " << readsMatch->readsInBits[i].ub << endl;
	}
	for(int i = 0;i < MAX_MAPPING_READS;i++)
	{
		for(int j = 0;j < 200;j++)
		{
			cout << " i j  = " << i << " " << j << " " << result[i].nMismatch[j] << " " << result[i].nStartPos[j] << endl;
		}
	}
	...
	for(int i = 0;i < MAX_MAPPING_READS;i++)
	{
		for(int j = 0;j < 200;j++)
		{
			cout << "d_result i j  = " << i << " " << j << " " << d_result[i].nMismatch[j] << " " << d_result[i].nStartPos[j] << endl;
		}               
	} 
	...
	CUDAMapping<<<BLOCKS, THREADS>>>(d_matchOpt, d_refGenome, d_hashTable,
...
void Matching(Option * opt, RefGenome * refGenome, HashTable * hashTable) {
