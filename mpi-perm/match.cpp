...
int StrCMPproteinSeq(const char * querySeq, const uint32_t & index,
                     const CReference * refGenome) {
...
uint32_t UpperBound(uint32_t low, uint32_t high, const char * querySeq,
                    const CReference * refGenome,
...
uint32_t LowerBound(uint32_t low, uint32_t high, const char * querySeq,
                    const CReference * refGenome,
                    const CHashTable * hashTable) {
  uint32_t mid = 0;
  while (low < high) {
