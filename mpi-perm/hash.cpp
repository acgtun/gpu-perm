...
void HashToBucket(const CReference * refGenome, CHashTable * hashTable) {
  INFO("The memory of Hash Table Index Array is",
       sizeof(uint32_t) * (hashTable->nSizeIndex / GB), "GB");
  MEMORY_ALLOCATE_CHECK(
      hashTable->index = (uint32_t *) malloc(
          sizeof(uint32_t) * hashTable->nSizeIndex));
  uint32_t hashValue;
  for (uint32_t i = 0; i < refGenome->nRefSize - HASHLEN; i++) {
    hashValue = GetHashValue(&(refGenome->refSeq[i]));
...
void hashTableTest(const Option & opt, const CReference * refGenome,
...
void WriteIndexAndRef(const Option & opt, const CReference * refGenome,
  ...
  fwrite(hashTable->counter, sizeof(uint32_t), hashTable->nSizeCounter + 1,
         fout);
...
void ReadIndexAndRef(const Option & opt, CReference * refGenome,
  ...
  MEMORY_ALLOCATE_CHECK(
      refGenome->refSeq = (char *) malloc(
          sizeof(char) * (refGenome->nRefSize + 1)));
  FREAD_CHECK(fread(refGenome->refSeq, sizeof(char), refGenome->nRefSize, fin),
              refGenome->nRefSize);
  ...
  MEMORY_ALLOCATE_CHECK(
      hashTable->counter = (uint32_t *) malloc(
          sizeof(uint32_t) * (hashTable->nSizeCounter + 1)));
  FREAD_CHECK(
      fread(hashTable->counter, sizeof(uint32_t), hashTable->nSizeCounter + 1,
            fin),
      hashTable->nSizeCounter + 1);
  ...
  MEMORY_ALLOCATE_CHECK(
      hashTable->index = (uint32_t *) malloc(
          sizeof(uint32_t) * hashTable->nSizeIndex));
  FREAD_CHECK(
      fread(hashTable->index, sizeof(uint32_t), hashTable->nSizeIndex, fin),
...
void SortEachBucket(const CReference * refGenome, CHashTable * hashTable) {
  ...
  for (uint32_t i = 0; i < NO_OF_BUCKET; i++) {
    if (hashTable->counter[i] == hashTable->counter[i + 1])
      continue;
    sort(hashTable->index + hashTable->counter[i],
         hashTable->index + hashTable->counter[i + 1], SortHashCMP);
  }
...
void BuildHashTable(const Option & opt, const CReference * refGenome,
  ...
  INFO("The size of Hash Table Counter Array is",
       sizeof(uint32_t) * (hashTable->nSizeCounter + 1) / GB, "GB");
...
void GenerateTestData(CReference * refGenome) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    for (int j = 0; j < 10000000; j++) {
      ...
      if (start + len[i] > refGenome->nRefSize - 10)
        continue;
