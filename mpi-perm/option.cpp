...
void GetReadLength(Option & opt) {
	...
	while (getline(fin, strLine)) {
		...
		if (opt.readLen != strLine.size()) {
			printf(
					"--ERROR INFO-- The lengths of the reads are not identical.\n");
			exit(EXIT_FAILURE);
...
bool isBuildIndex(const string strVal, const string strFind) {
	if(strVal.find(strFind) != string::npos) return true;
	return false;
}
...
void GetParameter(int argc, char* argv[], Option & opt) {
	...
	if (opt.refFile.size() > indexsf.size()
			&& opt.refFile.substr(opt.refFile.size() - indexsf.size())
			== indexsf) {
		opt.bIndexExist = 1;
	}
	...
	if ((GetStrVal(argc, argv, "-s", opt.indexFile) == 0
				&& opt.bSaveIndex == 1) || isBuildIndex(argv[0], "buildindex")) {
		string fileName;
		opt.bSaveIndex = 1;
	...
	if(opt.bSaveIndex != 1) GetReadLength(opt);
}
